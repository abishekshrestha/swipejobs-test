...
describe('App', () => {
  ...
  it('should match snapshot with mock API', async () => {
    let renderedComponent: any;

    await act(async () => {
      await axios.get.mockImplementation((url: string) => {
        if (url.endsWith('/profile')) return Promise.resolve(userMockData);
        if (url.endsWith('/matches')) return Promise.resolve(jobMockData);
        if (url.endsWith('/accept')) return Promise.resolve(acceptMockData);
        if (url.endsWith('/reject')) return Promise.resolve(rejectMockData);
      });
